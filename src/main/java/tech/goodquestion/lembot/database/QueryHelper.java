...
public final class QueryHelper {
    ...
    public static String NEXT_HIGHER_USER_AMOUNT_MONTHLY_BUMPS = "SELECT user_bump_time.id_discord, COUNT(user_bump_time.id_discord) AS num_bumps FROM `user_bump_time` INNER JOIN user_bump ON user_bump_time.id_discord = user_bump.id_discord WHERE bumped_at > (SELECT DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 MONTH)) GROUP BY user_bump_time.id_discord HAVING num_bumps > ? ORDER BY num_bumps ASC LIMIT 1;";
